...
class GroupInBatchSampler(Sampler):
    ...
    its own group.
    Args:
        batch_size(int) : samples_per_gpu.
    ...
    def __init__(
        ...
        assert hasattr(self.dataset, "_flag")
        self.flag = self.dataset._flag
        ...
        while True:
            ...
            for local_sample_idx in range(self.batch_size):
                ...
                if len(self.buffer_per_local_sample[local_sample_idx]) == 0:
                    ...
                    if self.dataset._keep_consistent_seq_aug:
                        self.aug_per_local_sample[local_sample_idx] = (
                            self.dataset.get_augmentation()
                        )

                if not self.dataset._keep_consistent_seq_aug:
                    self.aug_per_local_sample[local_sample_idx] = (
                        self.dataset.get_augmentation()
                    )
